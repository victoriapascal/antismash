...
def get_valid_areas(start: int, end: int, locations: Iterator[Location], start_overlap: int) -> list[tuple[int, int]]:
    ...
    for current in locations:
        if current.location.start < previous.location.end:  # overlaps with a previous CDS
            ...
            if current.strand == 1 and previous.strand == -1:
                ...
                add_area(current.location.start - start_overlap, previous.location.end + start_overlap)
            previous = current
            continue
        ...
        start = previous.location.end
        if previous.strand == -1:
            start -= start_overlap

        end = current.location.start
        if current.strand == 1:
            end += start_overlap
...
def run_tfbs_finder(record: Record, pvalue: float, start_overlap: int, matrix_path: str = PWM_PATH,
    ...
    for region in record.get_regions():
        ...
        areas: list[tuple[int, int]] = []
        if region.location.start > region.location.end:
            areas = get_cross_origin_areas(record, region, start_overlap)
        else:
            areas = get_valid_areas(region.location.start, region.location.end, iter(region.cds_children), start_overlap)
