    ...
    """ Finds the CDS features immediately next to, within, or containing the site with the
        ...
        Returns:
            a tuple of
                ...
                the CDS to the right (which may fully or partially contain the site) or None
    """
    if not region.location.start > region.location.end:
...
def add_neighbouring_genes(hit: dict[str, Any], left: Optional[CDSFeature], mid: Optional[CDSFeature],
    ...
    if left:
        hit["left"] = {
            "name": left.get_name(),
            "location": int(left.location.end),
    ...
    if mid:
        length = mid.location.end - mid.location.start
        if mid.location.start > mid.location.end:
            length = mid.location.start + mid.location.end
        hit["mid"] = {
            "name": mid.get_name(),
            "location": int(mid.location.start),
            "length": length,
    ...
    if right:
        hit["right"] = {
            "name": right.get_name(),
            "location": int(right.location.start),
